...
it('creates from valid string', function (string $countryCode, string $vatNumber): void {
    $vat = ValueAddedTaxRegistrationNumber::createFromString($vatNumber);

    expect($vat->countryCode)->toEqual($countryCode);
    expect($vat->registrationNumber)->toEqual($vatNumber);
